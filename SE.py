...
[data-testid="stAppViewContainer"] > .main {
    background-image: url("https://e1.pxfuel.com/desktop-wallpaper/780/115/desktop-wallpaper-physics-minimalist-math-minimalist.jpg");
    ...
    for i in range(num_equations):
        with equation_cols[2*i]:
            for j in range(num_equations):
                coeff_title = f"Coefficient of equation {i + 1}.{j + 1}"
                A[i, j] = st.number_input(coeff_title, format="%f", key=f"A_{i}_{j}")

        with equation_cols[2*i + 1]:
            const_title = f"Constant of equation {i + 1}"
